                    ...
                    if max_decrease_percent < 30 and \
                        ...
                        for k in range(j, n):
                            if bars[k].close < bars[j].close * 0.93:
                                print("fail 7%, bars[j].tradedate", bars[j].trade_date, bars[j].close, bars[k].trade_date, bars[k].close)
                                break

                            if bars[k].close > max_price:
                                max_price = bars[k].close
                                print("bars[j].tradedate", bars[j].trade_date, bars[k].trade_date, (max_price - bars[j].close) * 100.0 / bars[j].close)
